...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['admin_password'])) {
        ...
        if (!preg_match('/^[A-Za-z0-9._-]{3,40}$/', $username)) {
            ...
            try {
                $stmt = proxy_db()->prepare('INSERT INTO users (email,username,password_hash,api_key_hash,api_key_cipher,daily_request_limit,is_active) VALUES (?,?,?,?,?,?,1)');
                $stmt->execute([
                    strtolower($username) . '@proxy.local',
                    $username,
...
<?php else: ?><div class="top"><div><h1>Ollama Proxy Admin</h1><p class="muted">Admin-created accounts only. The generated API key is also the user's login password.</p></div><a href="?logout=1">Log out</a></div>
